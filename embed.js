...
module.exports = {
  ...
  error(chan, cont, title) {
    var message;
    var emb = new RichEmbed().setColor(COLORS.red).setDescription(cont);
    if (title) {
      emb.setTitle(title);
    }
  ...
  info(chan, cont, title) {
    var message;
    var emb = new RichEmbed().setColor(COLORS.blue).setDescription(cont);
    if (title) {
      emb.setTitle(title);
    }
    chan.send("", emb).then(m => {
  ...
  ok(chan, cont, title) {
    var message;
    var emb = new RichEmbed().setColor(COLORS.green).setDescription(cont);
    if (title) {
      emb.setTitle(title);
    }
    chan.send("", emb).then(m => {
  ...
  checkRights(msg, owner) {
    return (
      owner == msg.member.id || admins.find(x => x.id == msg.member.id) != null
    );
  }
};
